...
```{r plottitle, dependson = "title_word_pairs", fig.height=6, fig.width=9}
library(ggplot2)
library(igraph)
library(ggraph)
...
```{r plotdesc, dependson = "desc_word_pairs", fig.height=6, fig.width=9}
set.seed(1234)
desc_word_pairs %>%
  filter(n >= 5000) %>%
  graph_from_data_frame() %>%
...
```{r plotcounts, dependson = "toupper", fig.height=7, fig.width=9}
...
To examine the relationships among keywords in a different way, we can find the correlation among the keywords as described in Chapter 4. This looks for those keywords that are more likely to occur together than with other keywords.
...
```{r plotcors, dependson = "keyword_cors", fig.height=8, fig.width=12}
set.seed(1234)
keyword_cors %>%
...
```{r plottfidf, dependson = "full_join", fig.width=10, fig.height=7}
desc_tf_idf %>%
  filter(!near(tf, 1)) %>%
...
```{r plotbeta, dependson = "top_terms", fig.width=12, fig.height=16}
top_terms %>%
...
```{r plotgammaall, dependson = "lda_gamma", fig.width=7, fig.height=5}
...
```{r plotgamma, dependson = "lda_gamma", fig.width=10, fig.height=12}
...
Let's look specifically at topic 18 in Figure 8.8, a topic that had documents cleanly sorted in and out of it. There are many documents with $\gamma$ close to 1; these are the documents that *do* belong to topic 18 according to the model. There are also many documents with $\gamma$ close to 0; these are the documents that do *not* belong to topic 18. Each document appears in each panel in this plot, and its $\gamma$ for that topic tells us that document's probability of belonging in that topic.
...
```{r plottopkeywords, dependson = "top_keywords", fig.width=16, fig.height=16}
top_keywords %>%
  group_by(topic) %>%
  top_n(5, n) %>%
